...
template <typename T, uint N> Array<T, N>::Array() {
  data.resize(0);
  dims[0] = 0;
  dims[1] = 0;
  dims[2] = 0;
...
template <typename T> Array2D<T>::Array2D(uint inNDim0, uint inNDim1) {
...
template <typename T> matrix<T>::matrix(uint inNDim0, uint inNDim1) {
  this->data.resize(inNDim0 * inNDim1);
  this->dims[0] = inNDim0;
  this->dims[1] = inNDim1;
...
template <typename T> matrix<T>::matrix(const Array2D<T> &inMatrix) {
  this->data = inMatrix.data;
  for (uint i = 0; i < 4; i++)
    this->dims[i] = inMatrix.dims[i];
}

template <typename T, uint N> Array<T, N>::Array(const Array<T, N> &inMatrix) {
  data = inMatrix.data;
  for (uint i = 0; i < 4; i++)
  ...
  for (int i = dims[0] - 1; i >= 0; i--) {
    for (int j = dims[1] - 1; j >= 0; j--) {
      for (int k = dims[2] - 1; k >= 0; k--) {
        auto it =
            data.begin() + i * stride0 + j * stride1 + k * stride2 + offset;
...
template <typename T, uint N> void Array<T, N>::remove_dim_0(uint ind) {
...
template <typename T, uint N> void Array<T, N>::remove_dim_1(uint ind) {
...
template <typename T, uint N> void Array<T, N>::remove_dim_2(uint ind) {
...
template <typename T, uint N> void Array<T, N>::remove_dim_3(uint ind) {
...
template <typename T> void matrix<T>::addMatrix(matrix<T> &A, double a) {
#ifdef _DEBUG
...
template <> matrix<double> &matrix<double>::operator+=(matrix<double> &A) {
#ifdef _DEBUG
  if (A.dims[0] != this->dims[0] || A.dims[1] != this->dims[1])
    fatalErrorST("Incompatible matrix sizes for addMatrix.");
#endif
...
template <> matrix<double> &matrix<double>::operator*=(double a) {
...
template <typename T, uint N> T *Array<T, N>::operator[](int inRow) {
#ifdef _DEBUG
...
T &Array<T, N>::operator()(int i, int j, int k, int l) {
  ...
  if (i >= (int)this->dims[0] || i < 0 || j >= (int)this->dims[1] || j < 0 ||
    ...
    cout << "i=" << i << ", dim0=" << dims[0] << ", j=" << j
         << ", dim1=" << dims[1] << ", ";
    cout << "k=" << k << ", dim2=" << dims[2] << ", l=" << l
         << ", dim3=" << dims[3] << endl;
...
template <typename T> T &Array2D<T>::operator()(int i, int j) {
#ifdef _DEBUG
  if (i >= (int)this->dims[0] || i < 0 || j >= (int)this->dims[1] || j < 0) {
    cout << "i=" << i << ", dim0=" << this->dims[0] << ", j=" << j
         << ", dim1=" << this->dims[1] << endl;
...
template <typename T> T Array2D<T>::operator()(int i, int j) const {
#ifdef _DEBUG
  if (i >= (int)this->dims[0] || i < 0 || j >= (int)this->dims[1] || j < 0) {
    cout << "i=" << i << ", dim0=" << this->dims[0] << ", j=" << j
         << ", dim1=" << this->dims[1] << endl;
...
template <typename T> void matrix<T>::initializeToZero(void) {
  for (auto &val : this->data)
    val = 0;
}
...
template <typename T> void matrix<T>::timesMatrix(matrix<T> &A, matrix<T> &B) {
  uint i, j, k, p;
  p = A.dims[1];
#ifdef _DEBUG
  if (A.dims[0] != this->dims[1])
    fatalErrorST("Incompatible matrix sizes in matrix multiplication!");
#endif
  if (B.dims[0] != this->dims[0] || B.dims[1] != A.dims[1])
    B.setup(this->dims[0], A.dims[1]);
...
template <typename T> void matrix<T>::timesVector(vector<T> &A, vector<T> &B) {
#ifdef _DEBUG
  if (A.size() != this->dims[1])
    fatalError("Incompatible vector size");
#endif
  if (B.size() != this->dims[1])
    B.resize(this->dims[1]);
...
template <typename T> void Array2D<T>::appendRows(Array2D<T> &mat) {
#ifdef _DEBUG
  if (this->dims[1] != 0 && mat.getDim1() != this->dims[1])
    fatalErrorST("Attempting to append rows of wrong size to matrix.");
#endif
  this->data.insert(this->data.end(), mat.getData(),
                    mat.getData() + mat.getSize());

  if (this->dims[1] == 0)
    this->dims[1] = mat.getDim1();
...
void Array2D<T>::insertRow(const vector<T> &vec, int rowNum) {
  ...
  if (rowNum == INSERT_AT_END || rowNum == (int)this->dims[0]) {
    ...
    this->data.insert(this->data.begin() + rowNum * this->dims[1], vec.begin(),
                      vec.end());
  }

  if (this->dims[1] == 0)
    this->dims[1] = vec.size(); // This may not be needed (i.e. may never have
                                // this->dims[1]==0). need to verify how I set
                                // up this->dims[0], this->dims[1]...
  this->dims[0]++;
}
...
void Array2D<T>::insertRow(T *vec, int rowNum, int length) {
  ...
  if (rowNum == INSERT_AT_END || rowNum == (int)this->dims[0]) {
    ...
    // Insert at specified location
    this->data.insert(this->data.begin() + rowNum * this->dims[1], vec,
                      vec + length);
  }
...
template <typename T> void Array2D<T>::insertRowUnsized(const vector<T> &vec) {
  // Add row to end, and resize matrix (add columns) if needed
  if (vec.size() > this->dims[1])
    addCols(vec.size() - this->dims[1]);

  this->data.insert(this->data.end(), vec.begin(), vec.end());

  // If row too short, finish filling with 0's
  if (vec.size() < this->dims[1])
    this->data.insert(this->data.end(), this->dims[1] - vec.size(), (T)0);
...
template <typename T> void Array2D<T>::insertRowUnsized(T *vec, uint length) {
  // Add row to end, and resize matrix (add columns) if needed
  if (length > this->dims[1])
    addCols(length - this->dims[1]);

  this->data.insert(this->data.end(), vec, vec + length);

  // If row too short, finish filling with 0's
  if (length < this->dims[1])
    this->data.insert(this->data.end(), this->dims[1] - length, (T)0);
...
template <typename T> void Array2D<T>::addCol(void) {
...
template <typename T> void Array2D<T>::addCols(int nCols) {
  typename vector<T>::iterator it;
  for (uint row = 0; row < this->dims[0]; row++) {
...
template <typename T> void Array2D<T>::removeCols(int nCols) {
  if (nCols == 0)
    return;
...
template <typename T> vector<T> Array2D<T>::getRow(uint row) {
#ifdef _DEBUG
  if (row > this->dims[0])
    fatalErrorST("Attempting to grab row beyond end of matrix.");
#endif
  vector<T> out;
  out.assign(&(this->data[row * this->dims[1]]),
             &(this->data[row * this->dims[1]]) + this->dims[1]);
  return out;
}

template <typename T> Array2D<T> Array2D<T>::getRows(vector<int> ind) {
  matrix<T> out;
  for (auto &i : ind)
...
template <typename T> vector<T> Array2D<T>::getCol(int col) {
  vector<T> out;
  for (uint i = 0; i < this->dims[0]; i++)
    out.push_back(this->data[i * this->dims[1] + col]);
  return out;
}

template <typename T> Array2D<T> Array2D<T>::transpose(void) {
...
template <typename T> void matrix<T>::print(int prec) const {
  if (this->data.size() > 0) {
    cout << endl;
...
template <typename T> bool matrix<T>::checkNan(void) {
...
void matrix<T>::unique(matrix<T> &out, vector<int> &iRow) {
  ...
  // Setup vector for rows of final matrix that map to each row of initial
  // matrix
...
template <typename T, uint N> T *Array<T, N>::getData(void) {
  return data.data();
}

template <typename T> matrix<T> matrix<T>::invertMatrix(void) {
#ifdef _DEBUG
  ...
  for (uint k = 0; k < this->dims[0] - 1; k++) {
    ...
    for (uint i = k + 1; i < this->dims[0]; i++) {
      ...
      for (uint j = 0; j < this->dims[0]; j++) {
        ...
        identity(i, j) =
            identity(i, j) - ((first / input(k, k)) * identity(k, j));
      }
    }
...
template <typename T> vector<T> matrix<T>::solve(vector<T> RHS) {
#ifdef _DEBUG
  if (this->dims[0] != this->dims[1])
...
template <typename T> matrix<T> matrix<T>::adjoint(void) {
#ifdef _DEBUG
  if (this->dims[0] != this->dims[1])
...
template <typename T> T matrix<T>::frobNorm(void) {
  T norm = 0;
  for (auto &val : this->data)
    norm += val * val;
  return norm;
...
template <typename T> void matrix<T>::vecToMatrixResize(vector<T> &A) {
#ifdef _DEBUG
  if (A.size() != this->dims[0] * this->dims[1])
    fatalErrorST("Cannot fill vector into matrix - must have vector size == "
...
// Fix for compiler to know which template types will be needed later (and
// therefore must now be compiled):
