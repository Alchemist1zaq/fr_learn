...
void face::initialize(shared_ptr<ele> &eL, shared_ptr<ele> &eR, int gID,
                      int locF_L, faceInfo myInfo, input *params) {
  ID = gID;
...
void face::calcInviscidFlux(void) {
  if (!isMPI)
    getLeftState();
  this->getRightState(); // <-- makes this more general for all face types, and
                         // allows face memory to be contiguous
    ...
    for (int fpt = 0; fpt < nFptsL; fpt++) {
      ...
      if (isBnd) {
        if (isBnd > 1) {
          ...
          for (int dim = 0; dim < nDims; dim++) {
            for (int k = 0; k < nFields; k++) {
              Fc[dim][k] = tempFR[dim][k] + params->tau * normL(fpt, dim) *
                                                (UL(fpt, k) - UR(fpt, k));
            }
          }
        } else {
          ...
          for (int dim = 0; dim < nDims; dim++) {
            for (int k = 0; k < nFields; k++) {
              Fc[dim][k] = tempFL[dim][k] + params->tau * normL(fpt, dim) *
                                                (UL(fpt, k) - UR(fpt, k));
            }
          }
        }
        ...
        if (nDims == 2) {
          for (int k = 0; k < nFields; k++) {
            ...
            Fc[0][k] = 0.5 * (tempFL[0][k] + tempFR[0][k]) +
                       penFact * normX * (normX * (dF0) + normY * (dF1)) +
                       params->tau * normX * (dU);
            Fc[1][k] = 0.5 * (tempFL[1][k] + tempFR[1][k]) +
                       penFact * normY * (normX * (dF0) + normY * (dF1)) +
                       params->tau * normY * (dU);
          }
        } else if (nDims == 3) {
          for (int k = 0; k < nFields; k++) {
            ...
            Fc[0][k] = 0.5 * (tempFL[0][k] + tempFR[0][k]) +
                       penFact * normX *
                           (normX * (dF0) + normY * (dF1) + normZ * (dF2)) +
                       params->tau * normX * (dU);
            Fc[1][k] = 0.5 * (tempFL[1][k] + tempFR[1][k]) +
                       penFact * normY *
                           (normX * (dF0) + normY * (dF1) + normZ * (dF2)) +
                       params->tau * normY * (dU);
            Fc[2][k] = 0.5 * (tempFL[2][k] + tempFR[2][k]) +
                       penFact * normZ *
                           (normX * (dF0) + normY * (dF1) + normZ * (dF2)) +
                       params->tau * normZ * (dU);
          }
...
void face::rusanovFlux(void) {
  for (int fpt = 0; fpt < nFptsL; fpt++) {
    ...
    double rhoR = UR(fpt, 0);
    double uL = UL(fpt, 1) / rhoL;
    double uR = UR(fpt, 1) / rhoR;
    double vL = UL(fpt, 2) / rhoL;
    double vR = UR(fpt, 2) / rhoR;
    ...
    if (params->nDims == 2) {
      pL = (params->gamma - 1.0) *
           (UL(fpt, 3) - 0.5 * rhoL * (uL * uL + vL * vL));
      pR = (params->gamma - 1.0) *
           (UR(fpt, 3) - 0.5 * rhoR * (uR * uR + vR * vR));
    } else {
      wL = UL(fpt, 3) / rhoL;
      wR = UR(fpt, 3) / rhoR;
      pL = (params->gamma - 1.0) *
           (UL(fpt, 4) - 0.5 * rhoL * (uL * uL + vL * vL + wL * wL));
      pR = (params->gamma - 1.0) *
           (UR(fpt, 4) - 0.5 * rhoR * (uR * uR + vR * vR + wR * wR));
    }
    ...
    for (int i = 0; i < params->nFields; i++) {
      Fn(fpt, i) =
          0.5 * (tempFnL[i] + tempFnR[i] - eig * (UR(fpt, i) - UL(fpt, i)));
    }
...
void face::roeFlux(void) {
  ...
  for (int fpt = 0; fpt < nFptsL; fpt++) {
    ...
    if (nDims == 2) {
      pL = (gamma - 1.0) *
           (UL(fpt, 3) - (0.5 * UL(fpt, 0) * (vL[0] * vL[0] + vL[1] * vL[1])));
      pR = (gamma - 1.0) *
           (UR(fpt, 3) - (0.5 * UR(fpt, 0) * (vR[0] * vR[0] + vR[1] * vR[1])));
    } else {
      pL = (gamma - 1.0) *
           (UL(fpt, 4) - (0.5 * UL(fpt, 0) *
                          (vL[0] * vL[0] + vL[1] * vL[1] + vL[2] * vL[2])));
      pR = (gamma - 1.0) *
           (UR(fpt, 4) - (0.5 * UR(fpt, 0) *
    ...
    double eps =
        0.5 *
        (abs(rhoUnL / UL(fpt, 0) - rhoUnR / UR(fpt, 0)) +
         abs(sqrt(gamma * pL / UL(fpt, 0)) - sqrt(gamma * pR / UR(fpt, 0))));
    if (lambda0 < 2. * eps)
...
void face::laxFriedrichsFlux(void) {
  for (int fpt = 0; fpt < nFptsL; fpt++) {
    if (params->equation == ADVECTION_DIFFUSION) {
      ...
      double vNorm =
          params->advectVx * normL(fpt, 0) + params->advectVy * normL(fpt, 1);
      if (nDims == 3)
...
void face::centralFluxBound(void) {
  for (int fpt = 0; fpt < nFptsL; fpt++) {
    ...
    if (params->nDims == 2) {
      ...
      wR = UR(fpt, 3) / rhoR;
      pL = (params->gamma - 1.0) *
           (UL(fpt, 4) - rhoL * (uL * uL + vL * vL + wL * wL));
      pR = (params->gamma - 1.0) *
           (UR(fpt, 4) - rhoR * (uR * uR + vR * vR + wR * wR));
    }
