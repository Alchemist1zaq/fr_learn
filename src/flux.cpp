...
void inviscidFlux(const double *U, double F[3][5], input *params) {
  ...
  if (params->equation == ADVECTION_DIFFUSION) {
    ...
    if (params->nDims == 2) {
      ...
      double E = U[4];
      double p =
          (params->gamma - 1.0) * (E - 0.5 * rho * (u * u + v * v + w * w));
...
void viscousFlux(double *U, matrix<double> &gradU, double Fvis[3][5],
  ...
  if (!params->fixVis) {
    ...
    mu *= pow(rt_ratio, 1.5) * (1. + (params->c_sth)) /
          (rt_ratio + (params->c_sth));
  }
  ...
  if (nDims == 2) {
    ...
    dK_dx = 0.5 * (u * u + v * v + w * w) * dRho_dx +
            rho * (u * du_dx + v * dv_dx + w * dw_dx);
    dK_dy = 0.5 * (u * u + v * v + w * w) * dRho_dy +
            rho * (u * du_dy + v * dv_dy + w * dw_dy);
    dK_dz = 0.5 * (u * u + v * v + w * w) * dRho_dz +
            rho * (u * du_dz + v * dv_dz + w * dw_dz);
  }
  ...
  Fvis[0][1] = -tauxx;
  Fvis[0][2] = -tauxy;
  Fvis[0][nDims + 1] = -(u * tauxx + v * tauxy + w * tauxz +
                         (mu / params->prandtl) * (params->gamma) * de_dx);

  Fvis[1][0] = 0.0;
  Fvis[1][1] = -tauxy;
  Fvis[1][2] = -tauyy;
  Fvis[1][nDims + 1] = -(u * tauxy + v * tauyy + w * tauyz +
                         (mu / params->prandtl) * (params->gamma) * de_dy);
  ...
  if (nDims == 3) {
    ...
    Fvis[2][2] = -tauyz;
    Fvis[2][3] = -tauzz;
    Fvis[2][4] = -(u * tauxz + v * tauyz + w * tauzz +
                   (mu / params->prandtl) * (params->gamma) * de_dz);
  }
}
...
void ldgFlux(double *, double *, matrix<double> &, matrix<double> &, double *,
             input *) {
