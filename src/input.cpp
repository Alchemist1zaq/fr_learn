...
fileReader::fileReader(string fileName) { this->fileName = fileName; }

fileReader::fileReader(const fileReader &_fr) { this->fileName = _fr.fileName; }
...
void fileReader::setFile(string fileName) { this->fileName = fileName; }

void fileReader::openFile(void) {
  optFile.open(fileName.c_str(), ifstream::in);
}

void fileReader::closeFile() { optFile.close(); }
...
template <typename T> void fileReader::getScalarValue(string optName, T &opt) {
  string str, optKey;
  ...
  while (getline(optFile, str)) {
    ...
    if (optKey.compare(optName) == 0) {
      ...
      if (!(ss >> nVals)) {
        // This could happen if, for example, trying to assign a string to a
        // double
        cerr << "WARNING: Unable to read number of entries for vector option "
             << optName << endl;
        string errMsg = "Required option not set: " + optName;
        fatalError(errMsg.c_str());
      }
      ...
      for (int i = 0; i < nVals; i++) {
        if (!(ss >> opt[i])) {
          cerr << "WARNING: Unable to assign all values to vector option "
               << optName << endl;
          string errMsg = "Required option not set: " + optName;
          fatalError(errMsg.c_str())
        }
...
void input::readInputFile(char *filename) {
  ...
  case 6:
    ...
    if (order <= 2)
      RKa = {0., .153, .442, .931,
             1.0}; // Josh's secret-sauce P=2 optimal coeffs
    else
...
void input::nonDimensionalize(void) {
  ...
  PtBound = pBound * pow(1. + 0.5 * (gamma - 1.) * MachBound * MachBound,
                         gamma / (gamma - 1.));
