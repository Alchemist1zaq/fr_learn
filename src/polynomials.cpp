...
double Legendre(double in_r, int in_mode) {
  if (in_mode < 0)
    ...
    return ((2 * in_mode - 1) * in_r * Legendre(in_r, in_mode - 1) -
            (in_mode - 1) * Legendre(in_r, in_mode - 2)) /
           in_mode;
}
...
double dLegendre(double in_r, int in_mode) {
  ...
  if (in_mode == 0) {
    ...
    if (in_r > -1.0 && in_r < 1.0) {
      dLeg = in_mode *
             ((in_r * Legendre(in_r, in_mode)) - Legendre(in_r, in_mode - 1)) /
             (in_r * in_r - 1.0);
    } else {
      if (in_r == -1.0) {
...
double Legendre2D_hierarchical(int in_mode, vector<double> in_loc,
                               int in_basis_order) {
  double leg_basis = 0;
...
double eval_jacobi(double in_r, int in_alpha, int in_beta, int in_mode) {
  ...
  if (in_mode == 0) {
    ...
    jacobi = 0.5 * sqrt(dtemp_0 * (dtemp_1 / dtemp_2)) *
             sqrt(dtemp_3 / dtemp_4) * dtemp_5;
  } else {
    double dtemp_0, dtemp_1, dtemp_3, dtemp_4, dtemp_5, dtemp_6, dtemp_7,
        dtemp_8, dtemp_9, dtemp_10, dtemp_11, dtemp_12, dtemp_13, dtemp_14;

    dtemp_0 = in_mode * (in_mode + in_alpha + in_beta) * (in_mode + in_alpha) *
              (in_mode + in_beta);
    dtemp_1 = ((2 * in_mode) + in_alpha + in_beta - 1) *
              ((2 * in_mode) + in_alpha + in_beta + 1);
    dtemp_3 = (2 * in_mode) + in_alpha + in_beta;

    dtemp_4 = (in_mode - 1) * ((in_mode - 1) + in_alpha + in_beta) *
              ((in_mode - 1) + in_alpha) * ((in_mode - 1) + in_beta);
    dtemp_5 = ((2 * (in_mode - 1)) + in_alpha + in_beta - 1) *
              ((2 * (in_mode - 1)) + in_alpha + in_beta + 1);
    dtemp_6 = (2 * (in_mode - 1)) + in_alpha + in_beta;

    dtemp_7 = -((in_alpha * in_alpha) - (in_beta * in_beta));
    dtemp_8 = ((2 * (in_mode - 1)) + in_alpha + in_beta) *
              ((2 * (in_mode - 1)) + in_alpha + in_beta + 2);
...
double eval_grad_jacobi(double in_r, int in_alpha, int in_beta, int in_mode) {
  ...
  if (in_mode == 0) {
    ...
    grad_jacobi = sqrt(1.0 * in_mode * (in_mode + in_alpha + in_beta + 1)) *
                  eval_jacobi(in_r, in_alpha + 1, in_beta + 1, in_mode - 1);
  }
    ...
    for (k = 0; k < in_basis_order + 1; k++) {
      for (j = 0; j < k + 1; j++) {
        ...
        if (mode == in_mode) {
          ...
          dubiner_basis_2d =
              sqrt(2.0) * jacobi_0 * jacobi_1 * pow(1.0 - ab.y, i);
        }
      ...
      for (j = 0; j < k + 1; j++) {
        ...
        if (mode == in_mode) {
          ...
          if (i == 0) {
            ...
            dr_dubiner_basis_2d =
                2.0 * sqrt(2.0) * jacobi_0 * jacobi_1 * pow(1.0 - ab.y, i - 1);
          }
        }
    ...
    for (k = 0; k < in_basis_order + 1; k++) {
      for (j = 0; j < k + 1; j++) {
        ...
        if (mode == in_mode) {
          ...
          jacobi_3 =
              eval_grad_jacobi(ab.y, (2 * i) + 1, 0, j) * pow(1.0 - ab.y, i);
          jacobi_4 =
              eval_jacobi(ab.y, (2 * i) + 1, 0, j) * i * pow(1.0 - ab.y, i - 1);
          ...
          if (i == 0) {
            ...
            ds_dubiner_basis_2d =
                sqrt(2.0) *
                ((jacobi_0 * jacobi_1 * pow(1.0 - ab.y, i - 1) * (1.0 + ab.x)) +
                 (jacobi_2 * (jacobi_3 - jacobi_4)));
          }
        }
...
double compute_eta(int vcjh_scheme, int order) {
  ...
  if (order == 0 && vcjh_scheme != DG)
    fatalError(
        "ERROR: P=0 only compatible with DG. Set VCJH scheme type to 0!");
  ...
  if (vcjh_scheme == DG) {
    ...
    double ap = 1. / pow(2.0, order) * factorial(2 * order) /
                (factorial(order) * factorial(order));
    eta = c_1d * (2 * order + 1) / 2 * (factorial(order) * ap) *
          (factorial(order) * ap);
...
double VCJH_1d(double xi, int mode, int order, double eta) {
  ...
  if (mode == 0) // Left
    val = pow(-1, order) / 2. *
          (Legendre(xi, order) -
           (eta * Legendre(xi, order - 1) + Legendre(xi, order + 1)) /
               (1. + eta));
  else
    val = 0.5 *
          (Legendre(xi, order) +
...
double dVCJH_1d(double in_r, int in_mode, int in_order, double in_eta) {
  ...
  if (in_mode == 0) { // left correction function
    if (in_order == 0) {
      dtemp_0 = 0.5 * pow(-1.0, in_order) *
                (dLegendre(in_r, in_order) -
                 ((dLegendre(in_r, in_order + 1)) / (1.0 + in_eta)));
    } else {
      dtemp_0 = 0.5 * pow(-1.0, in_order) *
                (dLegendre(in_r, in_order) -
                 (((in_eta * dLegendre(in_r, in_order - 1)) +
                   dLegendre(in_r, in_order + 1)) /
                  (1.0 + in_eta)));
    }
  } else if (in_mode == 1) { // right correction function
    if (in_order == 0) {
      dtemp_0 = 0.5 * (dLegendre(in_r, in_order) +
                       ((dLegendre(in_r, in_order + 1)) / (1.0 + in_eta)));
    } else {
      dtemp_0 = 0.5 * (dLegendre(in_r, in_order) +
                       (((in_eta * dLegendre(in_r, in_order - 1)) +
