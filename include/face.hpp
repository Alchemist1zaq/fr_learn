...
public:
  ...
  void initialize(shared_ptr<ele> &eL, shared_ptr<ele> &eR, int gID, int locF_L,
                  struct faceInfo myInfo, input *params);
  ...
  /*! Viscous cases: For all internal faces, put the common solution into the
   * right ele Either put directly into ele's memory, or send across MPI
   * boundary */
  ...
  /*! Calculate the common flux using the Lax-Friedrichs method [scalar
   * advection] */
  ...
  vector<matrix<double>>
      FL; //! Flux matrix at each flux point [nFpts, nDims, nFields]
  vector<double *> FnL; //! Common normal flux for left ele (in ele's memory)
                        //! [nFpts, nFields]
  Array<double *, 2>
      dUcL; //! Common solution for left ele (in ele's memory)  [nFpts, nFields]
  matrix<double> Fn;    //! Common numerical flux at interface  [nFpts, nFields]
  matrix<double> normL; //! Unit outward normal at flux points
  vector<double>
      dAL; //! Local face-area equivalent (aka edge Jacobian) at flux points
  vector<double *> waveSp; //! Maximum numerical wave speed at flux point (in
                           //! left ele's memory)
  ...
  int isMPI; //! Flag for MPI faces to separate communication from flux
             //! calculation
