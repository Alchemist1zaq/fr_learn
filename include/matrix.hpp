...
template <typename T, uint N> class Array {
public:
...
template <typename T> class Array2D : public Array<T, 2> {
public:
  Array2D();
  ...
  /*! Insert a row into the Array at location rowNum [zero-indexed], with the
   * default being at the end */
...
template <typename T> class matrix : public Array2D<T> {
public:
  matrix();
