...
//! Shape function for linear or quadratic quad (TODO: Generalize to N-noded
//! quad)
...
//! Given points for a cell's face and a point inside the cell, get the outward
//! unit normal
...
//! Given a list of points, get an axis-aligned bounding box defined by its
//! centriod and dimensions
...
void refineGridBySplitting2D(matrix<int> &c2v, matrix<int> &c2f,
                             matrix<int> &f2v, vector<point> &xv,
                             vector<int> &parentCell, vector<int> &parentFace);

void refineGrid2D(geo &grid_c, geo &grid_f, int nLevels, int nNodes_c,
                  int shapeOrder_f);

//! Get reference location out_rst of point in_xyz within an element defined by
//! the points in xv
