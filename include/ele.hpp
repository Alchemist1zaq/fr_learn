...
public:
  ...
  int nMpts;  //! Number of nodes used for plotting (corners, not edge nodes for
              //! quadratic eles)
  ...
  ele(int in_eType, int in_order, int in_ID, geo *in_Geo);
  ...
  void calcTransforms_point(matrix<double> &jacobian, matrix<double> &JGinv,
  ...
  /*! Advance intermediate stages of Runge-Kutta time integration [With PMG
   * source term] */
  void timeStepA_source(int step, double rkVal);

  /*! Perform final advancement of Runge-Kutta time integration [With PMG source
   * term] */
  ...
  //! Get a bounding box for the element defined by the minimum and maximum
  //! extents
  ...
  // Other
  matrix<double>
      S_spts; //! Entropy-adjoint variable used as error indicator for Euler
  ...
  /* --- Overset Stuff --- */
  int sptOffset; //! Offset within overset data-transfer array to grab solution
                 //! data
  ...
  double &V_ppts(int ppt, int field);
