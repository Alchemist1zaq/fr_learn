...
enum ETYPE { TRI = 0, QUAD = 1, TET = 2, PRISM = 3, HEX = 4, PYRAMID = 5 };
...
enum NODE_TYPE { NORMAL_NODE = 0, OVERSET_NODE = 1, BOUNDARY_NODE = 2 };

/*! Enumeration for mesh (either create cartesian mesh or read from file) */
enum meshType { READ_MESH = 0, CREATE_MESH = 1, OVERSET_MESH = 2 };

enum EQUATION { ADVECTION_DIFFUSION = 0, NAVIER_STOKES = 1 };
...
enum VCJH_SCHEME { DG = 0, SD = 1, HU = 2, CPLUS = 3 };
...
struct point {
  ...
  double norm(void) { return std::sqrt(x * x + y * y + z * z); }
...
bool operator<(
    const point &a,
    const point &b); // Just a sort of 'dummy' function for sorting purposes
...
template <typename T> vector<int> findEq(const vector<T> &vec, T val) {
  vector<int> out;
...
template <typename T> int findFirst(vector<T> &vec, T val) {
  if (vec.size() == 0)
    return -1;
...
template <typename T> int findFirst(T *vec, T val, uint length) {
  if (length == 0)
    return -1;
...
template <typename T> void vecAssign(vector<T> &vec, vector<int> &ind, T val) {
  for (auto &i : ind)
    vec[i] = val;
}

// Good for numeric types - meant for ints or uints though
template <typename T> T getMax(vector<T> &vec) {
  T max = 0;
  for (auto &i : vec) {
    if (i > max)
      max = i;
...
template <typename T> T getMin(vector<T> &vec) {
  T min = 1e15;
  for (auto &i : vec) {
    if (i < min)
      min = i;
...
template <typename T> T getSum(vector<T> &vec) {
  T sum = 0;
...
template <typename T> void addVectors(vector<T> &vec1, vector<T> &vec2) {
  if (vec1.size() != vec2.size())
    fatalError("Vectors not of same size.");
...
template <typename T> vector<T> operator*(const vector<T> &lhs, double rhs) {
  vector<T> out(lhs.size());
  for (unsigned int i = 0; i < lhs.size(); i++)
    out[i] = lhs[i] * rhs;
...
vector<T> operator*(matrix<T> &mat, const vector<T> &vec) {
  if (mat.getDim1() != vec.size())
    fatalErrorST("Improper sizes for matrix*vector.");
